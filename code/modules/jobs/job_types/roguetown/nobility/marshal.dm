/datum/job/roguetown/marshal
	...
	tutorial = "You and your Councillors are the judge and jury, only overruled by His Majesty. \
				...
				After all, the Baron entrusted you with leading and maintaining his military might. \
				It may be the Baron's land, but you pull the strings in this town. \
				All in the name of fulfilling your duty to your Baron."
	whitelist_req = FALSE
...
/proc/find_lord(required_stat = CONSCIOUS)
	...
	for(var/mob/living/carbon/human/H in GLOB.human_list)
		if(!H.mind || H.job != "Baron" || (H.stat > required_stat))
			continue
		lord = H
		break
	return lord
...
/proc/lord_law_requested(mob/living/marshal, mob/living/carbon/human/lord, requested_law)
	...
	if(choice != "Yes" || QDELETED(lord) || lord.stat > CONSCIOUS)
		if(marshal)
			to_chat(marshal, span_warning("The Baron has denied the request for a new law!"))
		return
...
/proc/lord_law_removal_requested(mob/living/marshal, mob/living/carbon/human/lord, requested_law)
	...
	if(choice != "Yes" || QDELETED(lord) || lord.stat > CONSCIOUS)
		if(marshal)
			to_chat(marshal, span_warning("The Baron has denied the request for a law removal!"))
		return
...
/proc/lord_purge_requested(mob/living/marshal, mob/living/carbon/human/lord)
	...
	if(choice != "Yes" || QDELETED(lord) || lord.stat > CONSCIOUS)
		if(marshal)
			to_chat(marshal, span_warning("The Baron has denied the request for a purge of all laws!"))
		return
	purge_laws()
...
/proc/lord_outlaw_requested(mob/living/marshal, mob/living/carbon/human/lord, requested_outlaw)
	...
	if(choice != "Yes" || QDELETED(lord) || lord.stat > CONSCIOUS)
		if(marshal)
			to_chat(marshal, span_warning("The Baron has denied the request for declaring an outlaw!"))
		return
