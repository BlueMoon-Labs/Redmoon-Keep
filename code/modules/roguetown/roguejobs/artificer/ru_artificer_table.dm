/obj/machinery/artificer_table
	...
	desc = "Деревянная рабочая станция изобретателя, освященная каким-то странным механизмом или, возможно... магией..."
...
/obj/machinery/artificer_table/examine(mob/user)
	. = ..()
	if(material)
		. += span_warning("[initial(material.name)] уже готов к работе.")
...
/obj/machinery/artificer_table/attackby(obj/item/I, mob/living/user, params)
	...
	if(istype(I, /obj/item/rogueweapon/hammer))
		...
		if(material.artrecipe.progress == 100)
			...
			user.visible_message(span_info("[user] создает \a [created_item_instance.name]."))
		...
		if(skill < material.artrecipe.skill_level)
			if(prob(max(0, 25 - user.goodluck(2) - (skill * 2))))
				to_chat(user, span_warning("Ах да, моя неумелость приносит плоды."))
...
/obj/machinery/artificer_table/proc/choose_recipe(user)
	...
	var/i_type_choice = input(user, "Выбери тип", "Изобретатель") as null|anything in valid_types
	if(!i_type_choice)
		return
	...
	if(appro_recipe.len)
		var/datum/chosen_recipe = input(user, "Выбери создание", "Изобретатель") as null|anything in sortNames(appro_recipe.Copy())
