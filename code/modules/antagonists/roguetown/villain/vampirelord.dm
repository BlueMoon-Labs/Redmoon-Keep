...
/datum/antagonist/vampirelord/proc/grow_in_power()
	...
	switch(vamplevel)
		...
		if(3)
			...
			ascended = TRUE
			C.ascended = TRUE
			REMOVE_TRAIT(owner.current, TRAIT_CRITICAL_WEAKNESS, "[type]")
			for(var/datum/mind/thrall in C.vampires)
				if(thrall.special_role == "Vampire Spawn")
...
/datum/antagonist/vampirelord/proc/stake()
    if(staked)
        return
    staked = TRUE
...
/obj/structure/vampire/bloodpool/attack_hand(mob/living/user)
	...
	if(user.mind.special_role != "Vampire Lord")
		return
	var/choice = input(user,"What to do?", "REDMOON") as anything in useoptions|null
	switch(choice)
		if("Grow Power")
			if(lord.vamplevel == 4)
